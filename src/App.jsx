...
import { SoilMoistureProvider } from "./context/soilMoistureContext";
...
function App() {
  ...
  const [logs, setLogs] = useState([
    {
      type: "system",
      text: "Console initialized. Ready for commands.",
      timestamp: new Date(),
    },
    {
      type: "system",
  ...
  const executeCommand = (cmd) => {
    ...
    setTimeout(() => {
      ...
      if (cmdLower === "help") {
        response =
          "Available commands: status, moisture, temperature, water [plot_id], water all, refresh";
      } else if (cmdLower === "status") {
        response =
          "All systems operational. 9 plots monitored. 3 require attention.";
      } else if (cmdLower.startsWith("water")) {
        if (cmdLower === "water all") {
          response =
            "Initiating watering sequence for all plots. Please wait...";
          setTimeout(() => {
            setLogs((prev) => [
              ...prev,
              {
          ...
          response = plotId
            ? `Initiating watering sequence for plot ${plotId}...`
            : "Error: Please specify a plot ID.";
        }
      } else if (cmdLower === "moisture" || cmdLower === "temperature") {
        response = `Average ${cmdLower} readings: ${
